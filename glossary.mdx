...
A link from a figure in an answer back to the exact filing or transcript it came from. Open a citation to trace a number to its source. Citations are Kepler's core value — results are auditable through these links.
...
A pre-built starting point the connector ships with — surfaced as a slash command or quick action in some MCP clients. Prompts are a faster on-ramp to common tasks; you can always ask in your own words instead. See [Prompts](/mcp/prompts).
